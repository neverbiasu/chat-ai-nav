...
function PromptWorkshopPage() {
  ...
  return <PromptCardList templates={templates} onCardClick={showDetail} onFavorite={favoriteTemplate} />
}
  ...
  return (
    <div className="suggestions">
      {suggestedPrompts.map((prompt) => (
        <PromptCard key={prompt.id} data={prompt} variant="simple" onApply={() => applyPromptToChat(prompt.content)} />
      ))}
    </div>
  )
}
